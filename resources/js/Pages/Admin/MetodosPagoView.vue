...
const postBlog = handleSubmit(
  ...
  if (response.status === 201) {
        ...
        toast.add({ severity: 'success', summary: 'Realizado', detail: '¡Método de pago agregado correctamente!', life: 3000 });
    } else {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Error al agregar Método de pago. Intente mas tarde.', life: 3000 });
    }
...
const updateBlog = handleSubmit(
   async () => {
    ...
    if (response.status === 200) {
        ...
        toast.add({ severity: 'success', summary: 'Realizado', detail: '¡Método de pago actualizado correctamente!', life: 3000 });
    } else {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Error al actualizar Método de pago. Intente mas tarde.', life: 3000 });
    }
  ...
  confirm.require({
        ...
        accept: () => {
           const response =  BlogStore.deleteBlog({id: data.id,category: data.category});
            toast.add({ severity: 'warn', summary: 'Realizado', detail: '¡Método de pago borrado correctamente!', life: 3000 });
        },
        reject: () => {
...
<template>
  <div>
    <SectionTitle>
      <template #title>
        <p class=" text-2xl">Métodos de pago</p>
      </template>
      ...
      <div class="flex justify-end">
        <PrimaryButton @click="showDialog.newItem = !showDialog.newItem, cleanForm()">
          <label for="" v-if="!showDialog.newItem" class="flex justify-between gap-1">
            <li class="pi pi-plus"></li> Nuevo método de pago
          </label>
 ...
 -->
      ...
      <template v-if="showDialog.newItem">
        <Dialog v-model:visible="showDialog.newItem" maximizable modal header="Agregar nuevo metodo de pago" :style="{ width: '40rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
          <div class="flex flex-col items-start mb-4">
            <label for="title" class="font-semibold ">Titulo del método de pago:</label>
          ...
          <div class="flex flex-col items-start mb-4">
            <label for="image" class="font-semibold ">Imagen del método de pago</label>
      ...
      <template v-if="showDialog.updateItem">
        <Dialog v-model:visible="showDialog.updateItem" maximizable modal header="Actualizar metodo de pago" :style="{ width: '40rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
          <div class="flex flex-col items-start mb-4">
            <label for="title" class="font-semibold ">Titulo del método de pago:</label>
            <TextInput placeholder="Escriba el nombre" v-model="titleBlog" v-bind="titleBlogMeta" class="w-full" />
      ...
      <template v-if="showDialog.showItem">
        <Dialog v-model:visible="showDialog.showItem" modal header="Agregar nuevo metodo de pago" :style="{ width: '25rem' }">
          <div class="flex flex-col items-start mb-4">
            <label for="title" class="font-semibold ">Titulo del método de pago:</label>
            <h3>
